...
object EntityScan {
  def main(args: Array[String]): Unit = {
    ...
    // lets use mapPartition here
    val dataRows = hbaseRDD.mapPartitions(p => {
      val buffer = new ListBuffer[String]()
      while (p.hasNext){
        val tuple = p.next()
        val rowData = processRow(tuple._1, tuple._2)
        buffer+= JsonUtil.toJson(rowData)
      }
      buffer.iterator
    })

    dataRows.saveAsTextFile(cmds.output())
  }
  ...
  def processRow(key: ImmutableBytesWritable, result: Result): RowData = {
    ...
    try {
      ...
      case _: Throwable => {
        println(s"Houston, we have a problem near row $id")
        RowData(id, -1, -1, -1)
      }
    }
  }
...
class EntityOpts(args: Seq[String]) extends ScallopConf(args) {
